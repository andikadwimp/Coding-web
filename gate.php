...
$logo=$sets['logo_url']??'';
// NO REF handling di gate — gate murni buat tiktok ads atau direct visit
// Kalau lagi ada cookie ref (dari invite link sebelumnya), JANGAN hapus, biarkan

// Entry sticky: user yg landing di gate → cookie lx_entry=gate
// TAPI jangan override kalau user udah pernah pake invite (invite punya prioritas lebih tinggi)
if(empty($_COOKIE['lx_entry'])){
    setcookie('lx_entry','gate',time()+2592000,'/','',false,false);
}
...
<script>
// INSTANT REDIRECT: Kalau udah di standalone mode (PWA installed & dibuka dari icon)
// → langsung ke index, jangan tampilin gate install
(function(){
  if(window.matchMedia('(display-mode: standalone)').matches ||
     window.navigator.standalone === true ||
     document.referrer.indexOf('android-app://') === 0){
    // Set cookie pwa_app biar server juga skip gate
...
<script>
// NO ref handling di gate.php — cookie ref_code ga di-reset
...
// window.__refCode kosong di gate.php (no ref)
window.__refCode='';
</script>
